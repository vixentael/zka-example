      ...
      if let uid = Auth.auth().currentUser?.uid {
        ...
        do {
          if (isMyPost(postAuthor:postAuthor)) {
            postBody = try self.decryptBodyOfMyPost(encryptedBody: EncryptedData(base64String: postBody)!)
          } else {
            postBody = try self.decryptBodyOfOtherPost(encryptedBody: EncryptedData(base64String: postBody)!, author: postAuthor)
          }
        } catch {
