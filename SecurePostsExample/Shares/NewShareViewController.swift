...
class NewShareViewController: UIViewController, UITextFieldDelegate {
  ...
  @IBAction func didTapShare(_ sender: AnyObject) {
    ...
    ref.child("users").child(userID!).observeSingleEvent(of: .value, with: { (snapshot) in
      ...
      // [START_EXCLUDE]
      // Write new share
      self.writeNewShare(withUserID: userID!, username: user.username, body: self.bodyTextView.text)
  ...
  func writeNewShare(withUserID userID: String, username: String, body: String) {
    // Create new post at /user-posts/$userid/$postid and at
    // /posts/$postid simultaneously
    // [START write_fan_out]
    let key = ref.child("shares").childByAutoId().key
    let share = ["uid": userID,
                "author": username,
                "body": body]
    let childUpdates = ["/shares/\(key)": share,
                        "/user-shares/\(userID)/\(key)/": share]
