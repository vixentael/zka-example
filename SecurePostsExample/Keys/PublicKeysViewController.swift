...
class PublicKeysViewController: UIViewController, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    guard let publicKeyModel = Shares(snapshot: dataSource.snapshot(at: indexPath.row)) else { return }
  ...
  func getQuery() -> DatabaseQuery {
    let sharedKeysQuery = (ref?.child("public-keys").queryLimited(toFirst: 10))!
